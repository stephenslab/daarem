...
out <- system.time(fit1 <- mixem(L,x0,numiter = 250))
...
out <- system.time(fit2 <- mixdaarem(L,x0,numiter = 250))
...
f    <- mixobjective(L,x)
pdat <-
  rbind(data.frame(iter = 1:250,dist = f - fit1$value,method = "EM"),
        data.frame(iter = 1:250,dist = f - fit2$value,method = "DAAREM"))
